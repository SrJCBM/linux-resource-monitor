...
El men\'u principal y el historial se operan con opciones numeradas. Una entrada inv\'alida conserva el bucle activo y el filtro opcional exige una fecha real en formato \texttt{YYYY-MM-DD}. Antes de consultar, actualizar o eliminar, se presentan las capturas disponibles; cada fila muestra un n\'umero secuencial \texttt{N.} separado del ID persistente. Los IDs existentes no se renumeran y la secuencia se reinicia solo cuando el historial queda vac\'io. La eliminaci\'on exige una confirmaci\'on \texttt{SI} expl\'icita, aceptada sin distinguir may\'usculas ni acento. Los listados extensos se limitan inicialmente a 20 filas e indican cu\'antos registros se muestran respecto del total.
...
\midrule
Suite Windows & 69 pruebas; 2 omitidas; 0,261~s & 0 fallos \\
Suite Ubuntu WSL & 69 pruebas; 0 omitidas; 0,870~s & 0 fallos \\
...
La validaci\'on manual de Ubuntu produjo cinco hallazgos corregidos. Primero, el formato abreviado de \texttt{who} se normaliz\'o antes de calcular la duraci\'on en vivo. Segundo, la lectura hist\'orica de disco vuelve a convertir los GB almacenados al contrato de bytes esperado por la vista. Tercero, el filtro rechaza formatos y fechas calendario inv\'alidas en lugar de ejecutar una b\'usqueda vac\'ia. Cuarto, \texttt{N.} numera la presentaci\'on sin sustituir el ID estable; la secuencia solo vuelve a uno si se elimina la \`ultima captura. Quinto, consultar, actualizar y eliminar muestran primero el historial, y la confirmaci\'on \texttt{SI} sigue siendo expl\'icita pero ignora may\'usculas y el acento. La Fig.~\ref{fig:crud} conserva la evidencia diagn\'ostica previa a las correcciones, incluido el encabezado \texttt{ID} y la aparici\'on del ID~2 despu\'es de eliminar; no prueba la interfaz corregida. Ese comportamiento se comprueba mediante pruebas de regresi\'on y la suite completa en WSL.
