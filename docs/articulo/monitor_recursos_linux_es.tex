...
Cada parser acepta texto y puede probarse sin consultar el equipo actual. La red combina los contadores de \texttt{/proc/net/dev} con una sola direcci\'on por interfaz: prioriza la primera IPv4 y usa IPv6 como alternativa; si no existe una direcci\'on, conserva \texttt{None}. El disco analiza bytes de \texttt{df -P -B1}; al persistir convierte esos valores a las columnas en GB y, al consultar el historial, el repositorio los reconstruye en bytes antes de entregarlos a la vista. En usuarios, el instante abreviado informado por \texttt{who} se normaliza como \texttt{YYYY-MM-DD HH:MM}, incluso alrededor del cambio de a\~no; la duraci\'on se calcula al mostrar \texttt{inicio\_sesion} y no reemplaza el dato almacenado.
...
La verificaci\'on cubre tambi\'en contratos entre capas: que el controlador preserve errores parciales, que una captura incompleta sea rechazada, que las conexiones activen claves for\'aneas, que la actualizaci\'on afecte solo metadatos y que la eliminaci\'on retire las filas hijas. Para la interfaz se prueban fechas reales con formato estricto, identificadores no positivos, variantes de la confirmaci\'on expl\'icita, numeraci\'on visual, cancelaci\'on de borrado y mensajes de \texttt{EXITO}, \texttt{ADVERTENCIA} y \texttt{ERROR}.
...
El men\'u principal y el historial se operan con opciones numeradas. Una entrada inv\'alida conserva el bucle activo y el filtro opcional exige una fecha real en formato \texttt{YYYY-MM-DD}. Antes de consultar, actualizar o eliminar, se presentan las capturas disponibles en orden cronol\'ogico ascendente; cada fila muestra un n\'umero secuencial \texttt{N.} separado del ID persistente. Los IDs existentes no se renumeran y, si el historial est\'a vac\'io, cualquier secuencia residual se limpia antes de insertar para que la siguiente captura use ID~1. Una fecha v\'alida sin coincidencias se distingue de un historial vac\'io mediante un mensaje espec\'ifico. La eliminaci\'on exige una confirmaci\'on \texttt{SI} expl\'icita, aceptada sin distinguir may\'usculas ni acento. Los listados extensos se limitan inicialmente a 20 filas e indican cu\'antos registros se muestran respecto del total.
...
La Tabla~\ref{tab:resultados} resume la verificaci\'on automatizada y la validaci\'on manual final. Las suites se ejecutaron el 22 de julio de 2026 con \texttt{python -m unittest discover -s tests} en Windows y con \texttt{python3} en Ubuntu bajo WSL. En paralelo, un clon limpio del repositorio se recorri\'o directamente en Ubuntu mediante las nueve opciones del men\'u. Los tiempos corresponden a esas ejecuciones funcionales y no se presentan como mediciones de rendimiento.
...
\midrule
Suite Windows & 71 pruebas; 2 omitidas; 0,339~s & 0 fallos \\
Suite Ubuntu WSL & 71 pruebas; 0 omitidas; 0,615~s & 0 fallos \\
Estado general & 6 m\'odulos & 0 errores \\
CRUD SQLite & IDs 1--2, orden, edici\'on y borrado & Correcto \\
Concurrencia & 6 hilos + 1 hijo & Exit status 0 \\
\bottomrule
\end{tabular}
\end{table}

La opci\'on de estado general reuni\'o los seis m\'odulos en una sola consulta. Los paneles (a)--(c) de la Fig.~\ref{fig:evidencia-final} conservan las tres partes de esa salida: CPU y memoria; sistemas de archivos y red; y las listas de procesos y usuarios conectados. Los valores corresponden a la m\'aquina virtual observada: cinco procesadores l\'ogicos asignados, aproximadamente 4,8~GB de memoria, cinco montajes, dos interfaces y dos sesiones activas. Las consultas individuales de las opciones 2 a 7 se conservaron en el \'indice de evidencias del repositorio para contrastar cada tabla sin repetirlas en el art\'iculo.

\begin{figure*}[t]
\centering
\begin{minipage}[b]{0.315\textwidth}
\centering
\includegraphics[trim=0 0 0 220,clip,width=\linewidth]{../evidencias/capturas/05_opcion_1_estado_general_cpu_memoria.png}\\[-0.2em]
{\scriptsize (a) CPU y memoria.}
\end{minipage}\hfill
\begin{minipage}[b]{0.315\textwidth}
\centering
\includegraphics[width=\linewidth]{../evidencias/capturas/06_opcion_1_estado_general_disco_red.png}\\[-0.2em]
{\scriptsize (b) Disco y red.}
\end{minipage}\hfill
\begin{minipage}[b]{0.315\textwidth}
\centering
\includegraphics[width=\linewidth]{../evidencias/capturas/07_opcion_1_estado_general_procesos_usuarios.png}\\[-0.2em]
{\scriptsize (c) Procesos y usuarios conectados.}
\end{minipage}
\vspace{0.8em}

\begin{minipage}[b]{0.315\textwidth}
\centering
\includegraphics[trim=0 0 0 175,clip,width=\linewidth]{../evidencias/capturas/14_opcion_8_crud_registro_capturas.png}\\[-0.2em]
{\scriptsize (d) Registro de capturas 1 y 2.}
\end{minipage}\hfill
\begin{minipage}[b]{0.315\textwidth}
\centering
\includegraphics[trim=0 0 0 95,clip,width=\linewidth]{../evidencias/capturas/15_opcion_8_crud_listado_actualizacion.png}\\[-0.2em]
{\scriptsize (e) Listado y actualizaci\'on del ID 1.}
\end{minipage}\hfill
\begin{minipage}[b]{0.315\textwidth}
\centering
\includegraphics[width=\linewidth]{../evidencias/capturas/17_opcion_9_concurrencia_hilos_fork.png}\\[-0.2em]
{\scriptsize (f) Seis hilos y proceso hijo.}
\end{minipage}
\caption{Evidencia final ejecutada en Ubuntu: estado general (a)--(c), persistencia SQLite (d)--(e) y concurrencia (f).}
\label{fig:evidencia-final}
\end{figure*}
...
Los paneles (d)--(e) de la Fig.~\ref{fig:evidencia-final} muestran dos registros consecutivos con IDs 1 y 2. El listado presenta primero la captura de las 15:15:42 y despu\'es la de las 15:15:52, por lo que coincide con el orden cronol\'ogico ascendente del repositorio. La segunda vista conserva separados \texttt{N.} e \texttt{ID} y confirma la actualizaci\'on de la etiqueta del ID~1. La eliminaci\'on, el filtrado sin coincidencias, la consulta de detalle y el reinicio del identificador al vaciar el historial se verificaron durante el mismo recorrido y mediante pruebas de regresi\'on; la captura con el texto anterior del filtro se conserva solamente en el archivo de evidencias, no como figura final. El panel (f) registra seis hilos completados sin errores, PID padre e hijo distintos y c\'odigo de salida cero.
