...
La Tabla~\ref{tab:resultados} resume la validaci\'on final y la evidencia manual obtenida el 18 de julio de 2026 en Ubuntu. Las suites se ejecutaron con \texttt{python -m unittest discover -s tests} en Windows y con \texttt{python3} en Ubuntu bajo WSL; los dem\'as registros aplicaron el controlador de monitoreo, un repositorio SQLite temporal y la demostraci\'on de concurrencia. Los tiempos documentados corresponden a esas ejecuciones funcionales y no se presentan como mediciones de rendimiento.
...
\midrule
Suite Windows & 71 pruebas; 2 omitidas; 0,369~s & 0 fallos \\
Suite Ubuntu WSL & 71 pruebas; 0 omitidas; 0,751~s & 0 fallos \\
...
La evidencia de estado general registr\'o los seis m\'odulos con un diccionario de errores vac\'io; la Fig.~\ref{fig:estado-general} muestra CPU, memoria, disco y red en la terminal de Ubuntu. En la prueba de persistencia se cre\'o una captura, se list\'o, se actualizaron sus metadatos y se elimin\'o. La demostraci\'on de concurrencia produjo seis hilos sin errores, seguida de un hijo con PID distinto al padre y c\'odigo de salida cero, como se observa en la Fig.~\ref{fig:concurrencia}.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figuras/evidencia_estado_general_ubuntu.png}
\caption{Estado general observado en Ubuntu: CPU, memoria, sistemas de archivos y red.}
...
\caption{Evidencia diagn\'ostica CRUD previa a las correcciones, registrada durante la validaci\'on en Ubuntu.}
\label{fig:crud}
\end{figure*}

La validaci\'on manual de Ubuntu produjo cinco hallazgos corregidos. Primero, el formato abreviado de \texttt{who} se normaliz\'o antes de calcular la duraci\'on en vivo. Segundo, la lectura hist\'orica de disco vuelve a convertir los GB almacenados al contrato de bytes esperado por la vista. Tercero, el filtro rechaza formatos y fechas calendario inv\'alidas en lugar de ejecutar una b\'usqueda vac\'ia. Cuarto, \texttt{N.} numera la presentaci\'on sin sustituir el ID estable; el listado usa orden cronol\'ogico ascendente y la pr\'oxima captura vuelve a ID~1 siempre que el historial est\'e vac\'io. Quinto, consultar, actualizar y eliminar muestran primero el historial, y la confirmaci\'on \texttt{SI} sigue siendo expl\'icita pero ignora may\'usculas y el acento. La Fig.~\ref{fig:crud} conserva la evidencia diagn\'ostica previa a las correcciones, incluido el encabezado \texttt{ID} y la aparici\'on del ID~2 despu\'es de eliminar; no prueba la interfaz corregida. Ese comportamiento se comprueba mediante pruebas de regresi\'on y la suite completa en WSL.
...
\caption{Evidencia en Ubuntu de seis hilos sin errores y un proceso hijo con salida cero.}
